...
export class TemplatesService {
  ...
  async findOne(id: string) {
    const template = await this.templateRepository.findOne({
      where: { id },
      relations: ['blueprint', 'signatures', 'logos'],
    });
    if (!template) throw new NotFoundException('Template not found');
    return template;
  }

  async findOneByStructure(structureType: string, structureId: number) {
    const template = await this.templateRepository.findOne({
      // TODO: adicionar index para structureType e structureId
